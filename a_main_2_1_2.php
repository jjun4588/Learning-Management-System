...
<br>
			...
			$check2 = "UPDATE user_state SET ustate='재학' where id=$id";
			$result1 = mysql_query($check1, $connect);
			$result2 = mysql_query($check2, $connect);		
			$check = "SELECT  id, ustate, state FROM user_state where ustate='재학' or ustate='휴학';";
			...
			echo ("<form action='a_index_2_1.html' method='post'><table>
				<tr>
					...
					<th width='200px'> 등록 및 취소 </td>
			      	...
			      	for($i=0;$i<$count;$i++){
				if($sdtinfo[2]=='휴학 신청' || $sdtinfo[2]=='휴학 등록 완료'){
				echo("<tr>
