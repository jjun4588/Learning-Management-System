...
<br>
              <div class="cd3">
<br>
                  <p><span style="background: linear-gradient(to right, #ffa7a3, #5673bd); padding: 0.43em 1em; font-size: 19px; border-radius: 3px; color: #ffffff;"> 졸업요건 조회 </span></p><br>
                <p style="font-size: 23px; font-weight: bold; color: indianred; text-shadow: 1px 1px 1.2px midnightblue;">졸업가능 학점 : 6 이상 | 졸업가능 성적 : 3 이상 </p><br>

	<?php

	$con=mysql_connect('localhost','insik','123456')or die("mySQL 서버 연결 Error!");
	mysql_select_db("BachelorManagement", $con);
	$que = "select * from graduate";
	$res = mysql_query($que,$con);
	$anr = mysql_fetch_row($res);

	if($ans[0]>=$anr[0] and $ans[1]>=$anr[1])
	{ 
	echo $user_name."님은 "."<b>졸업 대상</b>"." 입니다. <br><br>";
	echo "현재 총 학점은 : ".$ans[0]." 입니다. <br>";
	echo "현재 평균 성적은 : ".$ans[1]." 입니다. <br>";
	}
	else
	{ 
	echo $user_name."님은 "."<b>졸업 대상이 아닙니다. </b><br><br>";
	echo "현재 총 학점은 : ".$ans[0]." 입니다. <br>";
	echo "현재 평균 성적은 : ".$ans[1]." 입니다. <br>";
	}
